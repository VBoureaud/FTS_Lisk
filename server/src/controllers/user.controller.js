...
const getUser = catchAsync(async (req, res) => {
  const addr = req.params.address ? req.params.address : req.query.address;
  const user = await userService.getUser(addr);
  if (user) {
    //const notifications = await notificationService.getNotification(req.query.address, req.query.server);
    return res.status(httpStatus.OK).send({ user: { ...user.toObject() } });
  }
