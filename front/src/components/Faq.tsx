...
export const faqData: { [key: string]: { category: string; content: string; } } = {
  ...
  'ETH': {
    category: 'What is it',
    content: `
      ETH (ETH) is a decentralized, public blockchain led by a global developer community. It’s fast, energy efficient, and reliable. With ease of development, low transaction costs, and a knowledgeable community, it provides developers with a strong open-source foundation for executing on the most demanding projects—without hurting the environment.
    `,
  },
  'ETH Account': {
    category: 'What is it',
    content: `
      Each ETH holder and sender of transactions is identified by an unique address (ex:rf1BiGeXwwQoi8Z2ueFYTEXSwuJYfV2Jpn). This address corresponds to the description of an "Account".
    `,
  },
  'Wallets': {
    category: 'What is it',
    content: `
      ...
      A public key is similar to an account number. If A wants to send some ETH to B when A sends the public key address to B. Anyone can send ETH using the public key.<br/> 
  ...
  'Uri': {
    category: 'What is it',
    content: `
      To minimize the footprint of NFTs without sacrificing functionality or imposing unnecessary restrictions, ETH NFTs do not have arbitrary data fields.<br/>
  ...
  'Create an account': {
    ...
    content: `
      On ETH there is not a dedicated "create account" transaction. The Payment transaction automatically creates a new account if the payment sends ETH equal to or greater than the   account reserve to a mathematically-valid address that does not already have an account. This is called funding an account. On testnet, you can use a faucet to generate an funded account. Read more about it here: https://ETH.org/ETH-testnet-faucet.html.<br/>
  ...
  'NFT transactions': {
    ...
    content: `
      ...
      When an offer is accepted, ETH are exchanged and the NFT owner change.<br/> 
  ...
  'DashBoard': {
    ...
    content: `
      ...
      - Current Balance: The number of ETH held by your account. If you don't have any more, you can no longer trade.<br/>
  ...
  'Collection Remote': {
    category: 'How to play',
    content: `
      After finding a profile from the Market page, you come to its collection page. It contains the list of his NFTs available as well as additional information such as his last connection, his ETH address, his badges and his statistics.
    `,
  },
  'Market': {
    category: 'How to play',
  ...
  'Privacy Policy': {
    ...
    content: `
      ...
      Create an account — when you sign up for and open an account on our application, we may ask you to provide us information such as a name, an ETH address and a location. This informations can be removed at any time by contacting us.<br/>
      Your Rights<br/>
...
const Faq: React.FunctionComponent<FaqProps> = (props) => {
  ...
  const filterData = (currentSearch: string, data: any) => {
    if (!data) return;
    if (currentSearch)
      return data.filter((e: Data, i: number) => i != 0 && e.filter((f: any) => f
        && (typeof f === 'string' || f instanceof String)
        && f.toLowerCase().indexOf(currentSearch.toLowerCase()) !== -1).length > 0)
    return data.filter((e: Data, i: number) => i != 0);
  }

  const handleRequestSort = (property: string) => {
    const isAsc = property && orderBy === property.toLowerCase() && order === 'asc';
