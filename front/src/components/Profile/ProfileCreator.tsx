...
export const ProfileCreator = (props: ProfileCreatorProps) => {
	...
	const [helpBox, setHelpBox] = React.useState<number[]>([]);
	...
	// params: { type: bla }
	const handleUpdate = (params: { [key: string]: string | undefined }) => {
		const profileCurrent: any = profile ? profile : {};
	...
	const handleConfirm = () => {
		setErrorMsg('');
		if (!props.burnout && profile && !profile.name) setErrorMsg('Name is missing');
		else if (profile && !profile.type) setErrorMsg('Type is missing');
		else if (!props.burnout && profile && !profile.location) setErrorMsg('Location is missing');
					...
					<Box sx={{ display: 'flex', flexWrap: 'wrap' }}>
						...
						*/}
						...
						<PictureCreator
							...
							level={props.burnout ? undefined : 1}
							type={profile.type}
							name={profile.name}
							...
							{preSelectProfil &&
								<Box sx={{ maxWidth: '930px', background: 'white', padding: 2, width: '100%', display: 'flex', flexWrap: 'wrap', marginLeft: '10px' }}>
									<Box sx={{ flex: 1 }}>
										<PictureCreator
											mode={0}
											imageDisplay={props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].image}
											type={props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].type}
										/>
									</Box>
									<Box sx={{ flex: 3, color: 'black', background: 'white', padding: 2, borderRadius: 5 }}>
										...
										<Box sx={{ mt: 1 }}>
											<Typography variant='body1'>Profession name : <span style={{ fontSize: '22px' }}>{props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].type}</span></Typography>
											<Typography variant='body1'>Description : {props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].description}</Typography>
										</Box>

										<Box sx={{ mt: 1 }}>
											<Typography sx={{ fontWeight: 'bold' }} variant='body1'>Specifications:</Typography>
											<Typography variant='body1'>Pocket capacity : {props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].pocketSize} items</Typography>
											...
											<Typography variant='body1'>Number of NFT (ingredient/recipe) you can create : {props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].typeCount}</Typography>
										</Box>
										...
										<Box sx={{ display: 'flex', mt: 5, justifyContent: 'center' }}>
											<Button
												onClick={() => handleUpdate({
													type: props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].type,
													image: props.listProfiles && props.listProfiles.filter(elt => elt.name == preSelectProfil)[0].image
												})}
												sx={{ color: 'white' }}
												variant="contained"
											>
