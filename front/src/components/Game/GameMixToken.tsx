...
const GameMixToken : React.FunctionComponent<GameMixTokenProps> = (props) => {
	...
	useEffect(() => {
		...
		else if (step == 1) { // gameStep
			myP5Canvas = new p5(sketch, p5Ref.current);
		} else if (step === 2) { // pre victory
			timeout = setTimeout(() => {
				if (myP5Canvas)
					myP5Canvas.remove();
				setStep(3);
			}, 2000);
		} else if (step === 3) { // victory
			timeout = setTimeout(() => {
				setStep(0);
		...
		<Box sx={{ width: '100%' }}>
			{step == 0 && 
				<Box onClick={handleLaunch}>
					...
					<Typography sx={{ ml: 1 }} variant="body1"><b>Labyrinth</b> - Play and try to win an NFT.</Typography>
				</Box>}
			{(step == 1 || step == 2) && <>
